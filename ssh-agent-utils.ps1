...
function Start-SshAgent() {
    ...
    Write-Host "Start-SshAgent:: SSH agent PID:", $agentPid -ForegroundColor Green
}
...
$agent = Get-SshAgent
if ($agent -eq 0) {
	Write-Host "Get-SshAgent:: Starting SSH agent..." -ForegroundColor Green
	Start-SshAgent		# Start agent
	Add-SshKey			# Add my default key
} else {
	Write-Host "Get-SshAgent:: SSH agent is running (PID $agent)" -ForegroundColor Green
}